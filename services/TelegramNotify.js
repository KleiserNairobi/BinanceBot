...
const NotifyTelegram = async (data) => {
  ...
  const content = urlencode(`
    ...
    __Profits:__ ${parseFloat(data.realProfits).toFixed(2)} ${data.market2} ${
    data.market2 < 0 ? "🔻" : "↗"
  }\\
    `).replace(/\./g, "\\.");
  ...
  try {
    await axios.get(
      `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_ID}/sendMessage?chat_id=${process.env.TELEGRAM_CHAT_ID}&parse_mode=MarkdownV2&text=${content}`
    );
  } catch (err) {}
};
